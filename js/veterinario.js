...
const agregarReporte = (event) => {
    ...
    axios({
        method: 'POST',
        url: 'https://backend1-eta-ebon.vercel.app/crear_reporte',
        data,
...
const cargarReportes = () => {
    axios({
        method: 'GET',
        url: 'https://backend1-eta-ebon.vercel.app/obtener_reportes',
    }).then(function (response) {
...
const editReportes = (button) => {
    ...
    axios.get(`https://backend1-eta-ebon.vercel.app/obtener_reporte/${id}`, {
        headers: {
...
btnEditBuscar.addEventListener('click', async () => {
    ...
    try {
        const response = await axios.get(`https://backend1-eta-ebon.vercel.app/buscar_usuario_vt/${busqueda}`);
